...
func _ready() -> void:
	...
	# ✅ reçoit les clics même si l’UI les consomme ensuite
...
#  (évite plusieurs cercles en même temps)
# ============================================================
...
func _open_upgrade_menu() -> void:
	# ✅ 0) Un seul cercle/menu à la fois : on ferme tous les autres "Tower"
	...
	# Ferme proprement un ancien menu local (sans toucher au cercle ici)
	_close_menu_only()

	# 1) Toujours tenter d'afficher la portée (feedback)
	_show_range(true)

	# 2) Si pas de scène d'upgrade : on reste en "portée seule" (ex: MK3 / tour max)
	...
	# 3) Verrouillage : on laisse la portée visible mais pas de menu
	...
	# 5) Watcher léger : si le menu quitte l'arbre, on coupe
	_start_range_watch()
...
func _is_menu_visible() -> bool:
	...
	if not (_menu_ref as Node).is_inside_tree():
		return false
	# ⚠️ Le menu peut "se cacher" par modulate/scale => visible peut rester true.
	# Le watcher est juste un filet de sécurité ; la fermeture principale est gérée par _input().
...
func _ensure_range_ring() -> void:
	...
	_range_ring.z_index = -1 # derrière la tour (si besoin ajuste)
	_range_ring.visible = false
	add_child(_range_ring)


func _get_detector_radius() -> float:
	# 1) Priorité : detector exporté (Area2D de portée)
...
func _is_click_on_menu(world_pos: Vector2) -> bool:
	...
	# ✅ Root fullscreen => background (overlay) => considéré "à côté"
	if best == root:
		return false
...
func _find_smallest_hit_control(root: Control, world_pos: Vector2) -> Control:
	...
	while not stack.is_empty():
		# ✅ pop_back() retourne Variant -> on caste en Control
...
class RangeRing extends Node2D:
	...
	func _draw() -> void:
		# Cercle rempli (bleu très léger)
		draw_circle(Vector2.ZERO, radius, Color(0.35, 0.65, 1.0, 0.12))
		# Contour un peu plus visible
