...
var _is_dead: bool = false         # bloque la mort multiple

# --- SPLIT LOGIC ---
@export var split_count: int = 0                # nombre d’enfants à spawn à la mort
@export var split_scene: PackedScene = null     # scène du mob enfant
@export var split_offset: float = 32.0          # distance d’écartement entre enfants
...
func _die() -> void:
	...
	# --- SPLIT LOGIC ---
	if split_scene and split_count > 0:
...
func _spawn_split_children() -> void:
	var follower := get_parent() as PathFollow2D
	if follower == null:
		push_warning("[Mob] Impossible de spawner les enfants : pas de PathFollow2D parent.")
		return

	var path := follower.get_parent() as Path2D
	if path == null:
		push_warning("[Mob] Impossible de spawner les enfants : pas de Path2D parent.")
		return

	for i in range(split_count):
		# Créer un nouveau PathFollow2D pour chaque enfant
		var new_follower := PathFollow2D.new()
		path.add_child(new_follower)
		new_follower.progress = follower.progress  # même position sur le chemin

		# Instancier le mob enfant
		var child: Node2D = split_scene.instantiate()
		new_follower.add_child(child)

		# Petit décalage visuel (triangle, carré…)
		var angle := (TAU / float(split_count)) * float(i)
		var offset := Vector2(cos(angle), sin(angle)) * split_offset
		...
		print("[Mob] Spawned child #%d at %s" % [i, child.global_position])
